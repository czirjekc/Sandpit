GO
/****** Object:  StoredProcedure [dbo].[sp_SWPnL_ImportProductsAndLicenses]    Script Date: 03/23/2011 14:14:01 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_SWPnL_ImportProductsAndLicenses]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_SWPnL_ImportProductsAndLicenses]

GO
/****** Object:  StoredProcedure [dbo].[sp_SWPnL_ImportProductsAndLicenses]    Script Date: 03/23/2011 14:14:12 ******/
SET ANSI_NULLS ON
GO
...
GO
/****** Object:  StoredProcedure [dbo].[sp_SWPnL_ProcessImports]    Script Date: 03/23/2011 14:14:41 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_SWPnL_ProcessImports]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_SWPnL_ProcessImports]

GO
/****** Object:  StoredProcedure [dbo].[sp_SWPnL_ProcessImports]    Script Date: 03/23/2011 14:15:00 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
WHERE 
(Products.Source NOT LIKE 'GMSr' AND Products.Source NOT LIKE 'OldGMS' AND Products.Source IS NOT NULL)
AND
(NOT EXISTS (SELECT * FROM R_SoftwareOrder WHERE Products.Id = SoftwareProductId))
...
WHERE (Products.Source NOT LIKE 'GMSr' AND Products.Source NOT LIKE 'OldGMS' AND Products.Source IS NOT NULL)
AND (NOT EXISTS (SELECT * FROM R_SoftwareLicense WHERE Imports.UniqueLicenseIdentifier = SourceId))
...
INNER JOIN R_SoftwareOrder Orders ON Orders.Id = Licenses.SoftwareOrderId
INNER JOIN R_SoftwareProduct Products ON Products.Id = Orders.SoftwareProductId
WHERE Imports.UniqueLicenseIdentifier IS NULL
AND Licenses.SourceId IS NOT NULL
...
AND R_SoftwareLicense.SourceId IS NOT NULL
...
WHERE Licenses.SourceId IS NOT NULL
AND
...
AND EXISTS (SELECT * FROM SWPnL_Import WHERE UniqueLicenseIdentifier = Licenses.SourceId)
...
WHERE EXISTS (SELECT * FROM SWPnL_Import WHERE UniqueLicenseIdentifier = SourceId)
...
WHERE 
NOT EXISTS (SELECT * FROM Olaf_Hard WHERE LOWER(ImportHardware.ComputerName) LIKE LOWER(OlafHardName))
AND
NOT EXISTS (SELECT * FROM R_UnknownHardware WHERE LOWER(ImportHardware.ComputerName + '.' + ImportHardware.SourceName) LIKE LOWER(R_UnknownHardware.MachineName + '.' + R_UnknownHardware.SourceName))
...
WHERE 
NOT EXISTS (SELECT * FROM Olaf_User WHERE PEO_USERID_CD IS NOT NULL AND LOWER(PEO_USERID_CD) LIKE LOWER(ImportUser.UserName))
AND
NOT EXISTS (SELECT * FROM R_UnknownUser WHERE LOWER(R_UnknownUser.UserName + '.' + R_UnknownUser.SourceName) LIKE LOWER(ImportUser.UserName + '.' + ImportUser.SourceName) )
...
WHERE 
NOT EXISTS (SELECT * FROM R_UnknownHardware WHERE R_UnknownHardware.MachineName LIKE Imports.ComputerName)
AND
NOT EXISTS (SELECT * FROM R_UnknownUser WHERE R_UnknownUser.UserName LIKE Imports.Username)
