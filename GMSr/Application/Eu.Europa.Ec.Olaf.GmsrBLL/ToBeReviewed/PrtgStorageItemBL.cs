...
{
    public class PrtgStorageItemBL
    {
        #region Properties
        ...
        public static List<PrtgStorageItemBL> GetAll()
        {
            PrtgStorageItemBL itemBL;
            List<PrtgStorageItemBL> itemList_BL = new List<PrtgStorageItemBL>();

            PrtgStorageItem.GetAll().ForEach(item =>
            {
                itemBL = new PrtgStorageItemBL();
                itemBL.MapData(item);
        ...
        public static void OpenXMLExport(HttpResponse response)
        {
            ...
            spreadSheetDocument = AddSheetAndData(spreadSheetDocument, "Lun storage", PrtgStorageItemBL.GetAll().ToList<object>());
        ...
        public static DocumentFormat.OpenXml.Packaging.SpreadsheetDocument AddSheetAndData(DocumentFormat.OpenXml.Packaging.SpreadsheetDocument spreadsheetDocument, string sheetName, List<object> itemList)
        {
            ...
            if (itemList.Count > 0)
            {
                itemList.ForEach(x =>
                {
                    ...
                    {
                        cell = new DocumentFormat.OpenXml.Spreadsheet.Cell();
                        if (x.GetType().GetProperty(y.Name).GetValue(x, null) != null)
                        {
                            tempValue = x.GetType().GetProperty(y.Name).GetValue(x, null).ToString();
                        ...
                        cell.CellValue = new DocumentFormat.OpenXml.Spreadsheet.CellValue(tempValue.ToString());
                        if (tempValue.GetType() == typeof(DateTime))
                        {
                            cell.CellValue = new DocumentFormat.OpenXml.Spreadsheet.CellValue(((DateTime)tempValue).ToOADate().ToString());
                            cell.StyleIndex = 2U;
                        }
                        else if (tempValue.GetType() == typeof(Double) || tempValue.GetType() == typeof(Int32) || tempValue.GetType() == typeof(Decimal))
                        {
                            cell.CellValue = new DocumentFormat.OpenXml.Spreadsheet.CellValue(tempValue.ToString());
        ...
        internal void MapData(PrtgStorageItem item)
        {
            VhdObjectId = item.VhdObjectId;
            VhdVhdPath = item.VhdVhdPath;
            VhdVhdSize = item.VhdVhdSize;
            VhdCurrentNode = item.VhdCurrentNode;

            VolumeClusterName = item.VolumeClusterName;
            VolumeLunId = item.VolumeLunId;
            VolumeObjectId = item.VolumeObjectId;
            VolumeVolumeName = item.VolumeVolumeName;

            LunAggregateName = item.LunAggregateName;
            LunAggregateSize = item.LunAggregateSize;
            LunAggregateUsedSpace = item.LunAggregateUsedSpace;
            LunLunId = item.LunLunId;
            LunLunPath = item.LunLunPath;
            LunLunSize = item.LunLunSize;
            LunLunVolumeName = item.LunLunVolumeName;
            LunLunVolumeSize = item.LunLunVolumeSize;
            LunLunVolumeUsedSpace = item.LunLunVolumeUsedSpace;
            LunObjectId = item.LunObjectId;
            LunSanName = item.LunSanName;
        }
