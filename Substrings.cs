...
class Result
{
    ...
    {
        List<int> matches = new List<int>(queries.Count);
        int index = 0;
        foreach(var searchItem in queries)
        {
            int count = 0;
            foreach(String target in stringList)
            {
                if(String.Equals(searchItem, target, StringComparison.InvariantCultureIgnoreCase))
                    count++;
            }
            matches.Add(count);
        }  
